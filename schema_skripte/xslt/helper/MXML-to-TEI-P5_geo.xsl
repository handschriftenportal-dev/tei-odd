                        ...
                        <xsl:element name="listRelation" namespace="http://www.tei-c.org/ns/1.0">
                            ...
                            <xsl:variable name="tempList">
                                <xsl:element name="list">
                                    <xsl:for-each select="//h1:Block[ @Type = 'geo' ]
                                        [h1:Field[ @Type = '219a' ][ @Value = 'GND-GEO-Nummer' ]
                                        [h1:Field[@Type = '219e'][@Value != 'nicht vorhanden']]]
                                        [h1:Field[ @Type = '2055' ][ (@Value = 'Ort') or (@Value = 'Ortsteil') or (@Value = 'Landschaft') ]]">
                                        <xsl:sort select="h1:Field[ @Type = '2055' ]/@Value"/>
                                        <xsl:sort select="h1:Field[ @Type = '2050' ]/@Value"/>">
                                        <xsl:element name="item">
                                            ...
                                            <xsl:if test="h1:Field[ @Type = '2119' ] and $principal-root//h1:Block
                                                [ @Type = 'geo' ]
                                                [ h1:Field[ @Type = '2050' ]
                                                [ @Value = current()/h1:Field[ @Type = '2119' ]/@Value ]
                                                [ h1:Field[ @Type = '219a' ][ @Value = 'GND-GEO-Nummer' ]]
                                                ]/h1:Field[ @Type = '219a' ][ @Value = 'GND-GEO-Nummer' ]/h1:Field[ @Type = '219e']/@Value">
                                                <xsl:element name="relation">
                                                    <xsl:value-of select="$principal-root//h1:Block[ @Type = 'geo' ]
                                                        [ h1:Field[ @Type = '2050' ][ @Value = current()/h1:Field[ @Type = '2119' ]/@Value ]]/h1:Field[ @Type = '219a' ][ @Value = 'GND-GEO-Nummer' ]/h1:Field[ @Type = '219e']/@Value"/>
                                                </xsl:element>
                                            </xsl:if>
                            ...
                            <xsl:for-each select="$tempList//relation">
                                <xsl:element name="relation" namespace="http://www.tei-c.org/ns/1.0">
                                    ...
                                    <xsl:attribute name="xml:id" select=" concat('containedInPlace_place_gnd_', preceding-sibling::idno, '_place_gnd_', .) "/>
                                </xsl:element>
                            </xsl:for-each>
    ...
    <xsl:template match="h1:Block[ @Type = 'geo' ]">
        ...
        <xsl:element name="place" namespace="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:choose>
                ...
                <xsl:when test="h1:Field[ @Type = '2055' ][ @Value = 'Ortsteil' ]">
                    <xsl:attribute name="type" select=" 'cityPart' "/>
                </xsl:when>
                ...
                <xsl:when test="h1:Field[ @Type = '2055' ][ @Value = 'Kontinent' ]">
                    <xsl:attribute name="type" select=" 'continent' "/>
                </xsl:when>
                <xsl:when test="h1:Field[ @Type = '2055' ][ @Value = 'Planet' ]">
                    <xsl:attribute name="type" select=" 'planet' "/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:message>2055 nicht vorhanden oder ungewöhnlicher Begriff: <xsl:value-of select="h1:Field[ @Type = '2000' ]/@Value"/></xsl:message>
                </xsl:otherwise>
            </xsl:choose>
