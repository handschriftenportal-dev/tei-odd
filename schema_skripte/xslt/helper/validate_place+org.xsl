...
<xsl:stylesheet 
    ...
    <xsl:variable name="listPlace_archive" select="document('../../../nachweis_daten/1_0_Normdaten/Archiv/hsp_TEI_Import_Orte.xml')"/>
    <xsl:variable name="listOrg" select="document('../../../nachweis_daten/1_0_Normdaten/hsp_TEI_Import_Koerperschaften.xml')"/>
    <xsl:variable name="listOrg_archive" select="document('../../../nachweis_daten/1_0_Normdaten/Archiv/hsp_TEI_Import_Koerperschaften.xml')"/>
    ...
    <xsl:template match="/">
        <xsl:for-each select="//doc">
            ...
            <xsl:variable name="messages">
                <xsl:value-of select="concat($crlf, tokenize(substring-after(@href, 'nachweis_daten/'), '/')[3])"/>
                <xsl:for-each select="$listPlaceDesc//item">
                    <xsl:if test="not($listPlace//tei:placeName = current()) and not($listPlace_archive//tei:placeName = current())">
                        <xsl:value-of select="concat($crlf, '   place not available in normdata file: &quot;', ., '&quot;')"/></xsl:if>
                </xsl:for-each>
                <xsl:for-each select="$listOrgDesc//item">
                    <xsl:if test="not($listOrg//tei:orgName = current()) and not($listOrg_archive//tei:orgName = current())">
            ...
            <xsl:if test="$messages != concat($crlf, tokenize(substring-after(@href, 'nachweis_daten/'), '/')[3])">
