...
<xsl:stylesheet 
    ...
    <xsl:variable name="geo" select="document('../nachweis_daten/1_0_Normdaten/hsp_TEI_Import_Orte.xml')"/>
                ...
                <xsl:element name="body" namespace="http://www.tei-c.org/ns/1.0">
                    <xsl:element name="listOrg" namespace="http://www.tei-c.org/ns/1.0">
                        ...
                        <xsl:apply-templates select="//h1:Block[ @Type = 'soz' ]
                            ...
                            [$geo//tei:placeName = h1:Field[ @Type = '4564' ]/@Value]"/>
                        ...
                        <xsl:element name="listRelation" namespace="http://www.tei-c.org/ns/1.0">
                            ...
                            <xsl:for-each select="//h1:Block[ @Type = 'soz' ]
                                ...
                                <xsl:element name="relation" namespace="http://www.tei-c.org/ns/1.0">
                                    ...
                                    <xsl:attribute name="xml:id" select="translate(concat('location_', concat('soz_', h1:Field[ @Type = '4500']/@Value), '_', h1:Field[ @Type = '4564' ]/(if (contains(@Value, '&amp;')) then normalize-space(substring-before(@Value, '&amp;')) else @Value)), ' /()&lt;&gt;,\.', '--')"/>
                                </xsl:element>
    ...
    <xsl:template match="h1:Field[ @Type = '499a' ][ (@Value = 'GND-Nummer') or (@Value = 'ISIL-Sigle') ]
        [h1:Field[ @Type = '499e' ][ (@Value != 'nicht vorhanden') and not(contains(@Value, ' ('))  and not(contains(@Value, '...')) ]]">
        <xsl:element name="idno" namespace="http://www.tei-c.org/ns/1.0">
            <xsl:choose>
