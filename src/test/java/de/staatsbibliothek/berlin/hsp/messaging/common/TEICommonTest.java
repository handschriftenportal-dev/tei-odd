...
class TEICommonTest {
  ...
  @Test
  void testInitien() throws Exception {
    Model model = ModelFactory.createDefaultModel();
    Path initienPath = Path.of("nachweis_daten/1_0_Normdaten/1_4_Themenbereiche/Initien.ttl");
    try (FileInputStream inputStream = new FileInputStream(initienPath.toFile())) {
      assertDoesNotThrow(() -> importRDF(inputStream, model));
    }
  }

  private static void importRDF(FileInputStream inputStream, Model model) throws  Exception{
    RDFParser.create()
        .source(inputStream)
        .lang(Lang.TURTLE)
